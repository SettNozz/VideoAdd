...
//        av_dict_set(&sc->opt, argv[2]+1, argv[3], 0);
//    }

    /* allocate the output media context */
    avformat_alloc_output_context2(&sc->oc, NULL, "flv", sc->filename);
    if (!sc->oc) {
    ...
    if (sc->fmt->video_codec != AV_CODEC_ID_NONE) {
        add_stream(&sc->video_st, sc->oc, &sc->video_codec, sc->fmt->video_codec);
        sc->have_video = 1;
        sc->encode_video = 1;
    }
    if (sc->fmt->audio_codec != AV_CODEC_ID_NONE) {
        add_stream(&sc->audio_st, sc->oc, &sc->audio_codec, sc->fmt->audio_codec);
        sc->have_audio = 1;
        sc->encode_audio = 1;
    }
    ...
    if (sc->have_video)
        open_video(sc->oc, sc->video_codec, &sc->video_st, sc->opt);

    if (sc->have_audio)
        open_audio(sc->oc, sc->audio_codec, &sc->audio_st, sc->opt);
...
void muxing_Close_stream(Stream_context *sc){
    ...
    /* Close each codec. */
    if (sc->have_video)
        close_stream(sc->oc, &sc->video_st);
    if (sc->have_audio)
        close_stream(sc->oc, &sc->audio_st);

    if (!(sc->fmt->flags & AVFMT_NOFILE))
        /* Close the output file. */
        avio_closep(&sc->oc->pb);
